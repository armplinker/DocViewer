...
namespace DocViewer
{
    public partial class MasterPageSingleMenu : System.Web.UI.MasterPage
    {
        ...
        {
            Logger.Debug("MasterPageSingleMenu Page_Load Event");

            if (FindControl("UCDEMO_RadPersistenceManagerProxy1") is RadPersistenceManagerProxy pmProxy1)
                pmProxy1.UniqueKey = "UCDEMORRPM1";

            if (FindControl("UCDTC_RadPersistenceManagerProxy1") is RadPersistenceManagerProxy pmProxy2)
                pmProxy2.UniqueKey = "UCDTCPM1";

            if ( FindControl("DVM_RadPersistenceManagerProxy1") is RadPersistenceManagerProxy pmProxy3)
                pmProxy3.UniqueKey = "DVMRPM1";
            var rpmContext = "";
            try
            {
                switch (IsPostBack)
        ...
        {
            Logger.Debug("MasterPageSingleMenu Page_Unload Event");
